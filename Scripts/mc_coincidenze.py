...
# ──────────────────────────────────────────────────────────────────────────────
D_A = 18.0          # [cm] distanza sorgente → rivelatore A (fisso)
D_B = 36.0          # [cm] distanza sorgente → rivelatore B (mobile)
R_A = 2.54/2          # [cm] raggio faccia cristallo NaI del rivelatore A
...
def run_simulazione(n_events:   int   = N_EVENTS,
    ...
    # Calcolo Teorico
    frazione_teorica = (1 - cos_max_A) / 2
    omega_teorico = 2 * np.pi * (1 - cos_max_A)
    
    # Calcolo Monte Carlo
    frazione_mc = n_trigger_globale / n_events
    omega_mc = frazione_mc * 4 * np.pi

    print(f"\n  --- STIMA ANGOLO SOLIDO RIVELATORE A ---")
    print(f"  Frazione sferica Teorica = {frazione_teorica:.6f}")
    print(f"  Frazione sferica MC      = {frazione_mc:.6f}")
    print(f"  Angolo solido Teorico    = {omega_teorico:.6f} sr")
    print(f"  Angolo solido MC         = {omega_mc:.6f} sr")
