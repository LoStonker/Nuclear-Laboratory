...
if __name__ == '__main__':
    ...
    if N_rimbalzati > 0:
        ...
        frazione_omega_A = 0.5 * (1.0 - D_trigger / np.sqrt(R_trigger**2 + D_trigger**2))
        omega_A_steradianti = frazione_omega_A * 4 * np.pi
    
        print("\n--- Parametri Geometrici ---")
        print(f"Rivelatore A (Trigger): Distanza = {D_trigger} cm, Raggio = {R_trigger} cm")
        print(f"Frazione di Angolo Solido (dOmega/4pi) = {frazione_omega_A:.6e}")
        print(f"Angolo Solido (dOmega) = {omega_A_steradianti:.6e} sr\n")
