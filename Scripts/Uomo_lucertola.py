...
def calcola_efficienza(area_ufloat, tempo_vivo, attivita, BR, DeltaOmega_4pi):
    """
    area_ufloat : ufloat con l'area netta del picco
    tempo_vivo  : float, tempo vivo in secondi
    attivita    : float, attività della sorgente in Bq
    BR          : float, branching ratio (es. 0.851)
    DeltaOmega_4pi : float, frazione di angolo solido (es. 0.00124)

    Restituisce:
    eps_ass, eps_intr : ufloat (assoluta e intrinseca)
    """
    rate = area_ufloat / tempo_vivo
    eps_ass = rate / (attivita * BR)
    eps_intr = eps_ass / DeltaOmega_4pi
    return eps_ass, eps_intr
...
def plot_multiple_spectra(
    ...
    """
    Plotta due o più spettri sovrapposti, con stile a gradini.

    Parametri
    ---------
    data_list : list
        Può contenere:
          - percorsi a file .dat (stringhe) → vengono letti con estrai_spettro()
          - tuple (canali, conteggi) già pronti
    labels : list di str, opzionale
        Etichette per la legenda; se non fornite vengono usati i nomi dei file.
    colors : list di colori, opzionale
        Se non specificati, Matplotlib usa la sua palette di default.
    log_scale : bool, default True
        Se True, usa scala logaritmica sull'asse y.
    normalize : bool, default False
        Se True, ogni spettro viene normalizzato a 1 (area totale) prima del plot,
        utile per confrontare la forma indipendentemente dal tempo di acquisizione.
    rebin_n : int, default 1
        Se >1, applica il rebinning (raggruppa `rebin_n` canali consecutivi) a ogni spettro.
    xlim, ylim : tuple (min, max), opzionali
        Limiti degli assi.
    title, xlabel, ylabel : str
        Impostazioni del grafico.
    save_path : str, opzionale
        Se fornito, salva il grafico su file.
    show : bool, default True
        Se True, mostra il grafico a schermo.

    Restituisce
    -----------
    fig, ax : oggetti Matplotlib
    """

    n = len(data_list)
    if n < 2:
        raise ValueError("Devi fornire almeno due spettri per il confronto.")

    # Palette di default (puoi personalizzarla)
    if colors is None:
        # Usa i colori di default di matplotlib fino a 10 spettri, poi ripete
        colors = plt.cm.tab10(np.linspace(0, 1, n))

    # Genera etichette se non fornite
    if labels is None:
        labels = []
        for i, data in enumerate(data_list):
            if isinstance(data, str):
                labels.append(os.path.basename(data))
            else:
                labels.append(f"Spettro {i+1}")
    ...
    for i, data in enumerate(data_list):
        # 1. Carica i dati
        if isinstance(data, str):
            canali, conteggi = estrai_spettro(data, log_scale=False, show_plot=False)
            if canali is None or conteggi is None:
                print(f"Attenzione: impossibile leggere {data}, lo salto.")
                continue
        else:
            canali, conteggi = data

        # 2. Rebinning opzionale
        if rebin_n > 1:
            canali, conteggi = rebin(canali, conteggi, n=rebin_n)

        # 3. Normalizzazione
        if normalize:
            area = np.sum(conteggi)
            if area > 0:
                conteggi = conteggi / area
            else:
                print(f"Attenzione: area zero per {labels[i]}, spettro non normalizzato.")
    ...
    if log_scale:
        ax.set_yscale('log')
        # Imposta un limite inferiore ragionevole
        y_min = ax.get_ylim()[0]
        min_bottom = 1e-6 if normalize else 0.5
        ax.set_ylim(bottom=max(y_min, min_bottom))
...
class FitLikelihoodBomberone:
    ...
    def perform_fit(self, silent=False):
        ...
        try:
            ...
            if 'A_largo' in self.initial_params: self.minuit.limits['A_largo'] = (0.0, None)
            if 'sigma_largo' in self.initial_params: self.minuit.limits['sigma_largo'] = (0.1, None)
